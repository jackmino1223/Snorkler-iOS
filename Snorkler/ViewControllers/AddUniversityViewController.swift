...
class AddUniversityViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        */
        ...
        ApiHelper.getUniversity(onsuccess: { json in
            
            self.hideLoading()
            
            guard let resultJson = json?.arrayValue else { return }
            resultJson.forEach { resultJson in
                let item = School(schoolId: "", schoolName: resultJson["name"].stringValue)
                self.schoolsArray.append(item)
            }
            self.schoolsArray = self.schoolsArray.sorted { $0.schoolName < $1.schoolName }
            print(self.schoolsArray)
            DispatchQueue.main.async {
                self.schoolPicker.schoolsData = self.schoolsArray
                self.activeSchoolPicker(false)
