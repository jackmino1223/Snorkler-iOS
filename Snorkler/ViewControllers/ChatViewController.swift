...
final class ChatViewController: JSQMessagesViewController{
    ...
    func observeMessages() {
        ...
        DispatchQueue(label: "com.snorkler.loadingmessage").async {
            if self.newMessageRefHandle == nil {
                self.initMessagesHandler()
            }
        }
    }
    
    func initMessagesHandler() {
        let messageQuery = self.messageRef.queryLimited(toLast:30)
        ...
        self.newMessageRefHandle = messageQuery.observe(.childAdded, with: { (snapshot) -> Void in
            ...
            if let id:String = messageData["senderId"], let text:String = messageData["text"], text.characters.count > 0 {
...
extension ChatViewController:JSQMessageAvatarImageDataSource {
    func avatarImage() -> UIImage! {
        return UIImage(named: "profile pic")!
    }
