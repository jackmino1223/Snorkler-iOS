...
//                        let country:String = addressDict["Country"] as? String ?? ""
                        let zipCode:String = place.postalCode ?? "92626"
...
//                        let address = ([district, city, country] as [String]).joined(separator: ", ")
                        ...
                        AppSession.shared.zipCode = zipCode
                        UserDefaults.standard.set(AppSession.shared.zipCode, forKey: "zipcode")
                    }
    ...
    @IBAction func loginButtonDidTouch(_ sender: Any) {
        ...
        func onLogInSuccess() {
            ...
            UserDefaults.standard.set(AppSession.shared.userInfo?.memberId, forKey: "memberId")
        ...
        ApiHelper.signin(email:emailField.text ?? "", password:passwordField.text ?? "", onsuccess: { [weak self] (result) in
            ...
            if( AppSession.shared.userInfo?.memberId != nil && AppSession.shared.userInfo?.memberId != "") {
                ...
                let alert = UIAlertController(title: "Login Failed", message: "Looks like you entered the wrong username or password.", preferredStyle: UIAlertControllerStyle.alert)
