...
//let video_server = "http://localhost:8080"
...
class VideoChatViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.clean { done in
            ...
            self.getSessionCredentials(username: self.mySession.key)
            
            let listKey = AppSession.onlineUsers.map { return $0.email }
            listKey.forEach { (key) in
                self.getSessionCredentials(username: key)
            }
        }
    }
    ...
    private func clean(_ ready: ((Bool)->())? = nil) {
        Alamofire.request(video_server + "/session/clear", method: .post)
            .responseJSON { response in
            ErrorHelper.apiError(response.error)
    ...
    private func getSessionCredentials(username akey:String, onReady ready: ((Bool)->())? = nil) {
        ...
        Alamofire.SessionManager.default.requestWithoutCache(video_server + "/session/" + akey)
