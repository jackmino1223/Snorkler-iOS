...
class ApiHelper: Any {
    ...
    class func getTrendingInterests(onsuccess success:@escaping (JSON?)->(), onfailure fail:@escaping (String?)->()) {
        Alamofire.request(ApiHost + apiGetTwitterTrends + AppSession.shared.zipCode!, method: .get, parameters: nil).validate().responseJSON { response in
            resultClasssify(with: response, forType: .getTrendingInterests, onsuccess: success, onfailure: fail)
        }
    }
    
//    class func getTwitterInterests(onsuccess success:@escaping (JSON?)->(), onfailure fail:@escaping (String?)->()) {
//
//        Alamofire.request(ApiHost + apiGetTwitterTrends + AppSession.shared.zipCode!, method: .get, parameters: nil).validate().responseJSON { response in
//            resultClasssify(with: response, forType: .getTrendingInterests, onsuccess: success, onfailure: fail)
//        }
//    }
    ...
    class func getUniversity(onsuccess success:@escaping (JSON?)->(), onfailure fail:@escaping (String?)->()) {
        Alamofire.request(ApiHost + apiGetUniversity, method: .get, parameters: nil).validate().responseJSON { response in
            resultClasssify(with: response, forType: .getSchools, onsuccess: success, onfailure: fail)
        }
    }
    ...
    class func updateProfilePicture(memberId member_id: String,
...
//        }
        ...
        Alamofire.upload(multipartFormData: { (form) in
            ...
            form.append(data(using: .utf8)!, withName: "dp")
